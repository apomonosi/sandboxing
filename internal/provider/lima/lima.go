// Package lima will hold agentctl's macOS backend, shelling out to
// `limactl` the same way the Incus backend shells out to `incus`. For
// this milestone it is a capability-table-driven stub: every method
// returns the sentinel error matching its Capabilities() entry, so the
// CLI already behaves correctly (and demoably) on macOS before real
// `limactl` integration exists.
package lima
...
// New builds the Lima stub provider. It intentionally does not check for
// a `limactl` binary yet, since no method does real work.
