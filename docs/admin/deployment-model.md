...
2. Install the right backend per OS (see the provider setup pages) —
   currently only the Incus/Linux path is fully implemented; macOS/Windows
   users will see `UnderDevelopment` responses for lifecycle commands until
   their backends land (check the
